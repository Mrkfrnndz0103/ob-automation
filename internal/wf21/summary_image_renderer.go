...
import (
	...
	"wf21-drive-csv-consolidation/internal/seatalk"
...
func sendSummarySnapshotToSeaTalk(ctx context.Context, cfg workflowConfig, sheetsSvc *sheets.Service) (summaryImageSendResult, error) {
	...
	// Never skip send due to temporary instability: keep waiting until the
	// snapshot settles, or until the run context is canceled.
	for !stable {
		if err = waitWithContext(ctx, cfg.SummaryStabilityWait); err != nil {
			return result, err
		}
		stable, err = waitForStableSummarySnapshot(ctx, cfg, sheetsSvc)
		if err != nil {
...
func updateSummarySyncCell(ctx context.Context, cfg workflowConfig, sheetsSvc *sheets.Service) error {
	...
	if strings.TrimSpace(previousValue) == timestamp {
		// Timestamp format is minute-granularity; bump one minute to guarantee change.
		timestamp = formatSummarySyncTimestamp(now.Add(1 * time.Minute))
	}
...
func formatSummarySyncTimestamp(ts time.Time) string {
	// Target format requested by operations: mm-dd hh:mm (24-hour clock).
...
func waitForStableSummarySnapshot(ctx context.Context, cfg workflowConfig, svc *sheets.Service) (bool, error) {
	...
	for _, target := range targets {
		target := target
		stable, err := waitForStableDigest(ctx, cfg.SummaryStabilityRuns, cfg.SummaryStabilityWait, func(ctx context.Context) (string, error) {
			if len(target.Ranges) <= 1 {
				return captureStyledRangeDigest(ctx, svc, cfg.SummarySheetID, target.Tab, target.Ranges[0])
			}
			return captureConnectedStyledRangesDigest(ctx, svc, cfg.SummarySheetID, target.Tab, target.Ranges)
		})
		if err != nil {
			return false, err
		}
...
func captureConnectedStyledRangesDigest(ctx context.Context, svc *sheets.Service, sheetID, tab string, ranges []string) (string, error) {
	data, err := readConnectedStyledRanges(ctx, svc, sheetID, tab, ranges)
	if err != nil {
		return "", err
	}
	return summarySnapshotDigest(data)
...
type summarySnapshotTarget struct {
	Tab    string
	Ranges []string
}

func buildSummarySnapshotTargets(cfg workflowConfig) []summarySnapshotTarget {
	targets := []summarySnapshotTarget{
		{
			Tab:    cfg.SummaryTab,
			Ranges: []string{cfg.SummaryRange},
		},
	}
	if cfg.SummarySecondEnabled && len(cfg.SummarySecondRanges) > 0 {
		ranges := make([]string, 0, len(cfg.SummarySecondRanges))
		for _, rng := range cfg.SummarySecondRanges {
		...
		if len(ranges) > 0 {
			targets = append(targets, summarySnapshotTarget{
				Tab:    cfg.SummarySecondTab,
				Ranges: ranges,
			})
		}
	}
